        ...
        async function cargarClinicas() {
            ...
            function ClinicaElim(id) {
                async function elis(id) {
                    const ventana= window.confirm("¿Está seguro de eliminar?");
                    if (!ventana) return;

                    const delet= await llamadosClinic.deleteClinicas(id);
                    if(!delet){
                        setClinicas(prev => prev.filter(clinicas => clinicas.id !== id));
                         Swal.fire("clinica eliminada", "", "success");
                    } else {
    ...
    return(
        <div className='body8'>
            <div className='container88'>
                <p className='tit'>Panel de Control</p>
                <br /><br />
            <>
            ...
            <div className='render'>
                {clinicas.map((clinica) => (
                    <ul key={clinica.id}>
                     <br />
                     <strong>Nombre Clinica:</strong> {clinica.nombre_Clinica} <br />
